...
#saving the models in cache and loading
import os
...
class LoRA_Linear(nn.Module):
  def __init__(self,existing_linear,rank,input_size=768,output_size=768):
    #we will get input x as (512,768) @ W_q (768,768) -> (512,768) now W_q is frozen
    super().__init__()
    self.linear = existing_linear
    for params in self.linear.parameters():
      params.requires_grad = False

    self.A = nn.Parameter(torch.randn(rank, input_size).to(existing_linear.weight.device))
    self.B = nn.Parameter(torch.zeros(output_size, rank).to(existing_linear.weight.device))
    #not focus on scaling alpha and rank

  def forward(self,x):
    frozen_part = self.linear(x) #x @ W_q
    # updated = x @ (768,rank) @ (rank,768) = (512,768)
    return frozen_part + x @ self.A.T @ self.B.T

for param in model.parameters():
    param.requires_grad = False

for i in range(12):
  model.bert.encoder.layer[i].attention.self.query = LoRA_Linear(model.bert.encoder.layer[i].attention.self.query,rank=4,input_size=768,output_size=768)
  model.bert.encoder.layer[i].attention.self.value = LoRA_Linear(model.bert.encoder.layer[i].attention.self.value,rank=4,input_size=768,output_size=768)
